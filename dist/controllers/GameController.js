...
class GameController {
    ...
    async startGame(req, res) {
        try {
            ...
            // Start game
            const gameResponse = await GameService_1.gameService.startGame(userId);
            res.status(201).json({
    ...
    async stopGame(req, res) {
        try {
            ...
            // Stop game
            const gameResponse = await GameService_1.gameService.stopGame(userId, sessionId);
            res.status(200).json({
    ...
    async getUserStats(req, res) {
        try {
            ...
            // Get stats
            const stats = await GameService_1.gameService.getUserStats(userId);
            res.status(200).json({
    ...
    async getActiveSession(req, res) {
        try {
            ...
            // Get active session
            const activeSession = await GameService_1.gameService.getActiveSession(userId);
            if (!activeSession) {
                res.status(200).json({
    ...
    async cleanupExpiredSessions(req, res) {
        try {
            const cleanedCount = await GameService_1.gameService.cleanupExpiredSessions();
            res.status(200).json({
